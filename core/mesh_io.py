...
def default_mesh() -> tuple[trimesh.Trimesh, MeshInfo]:
    """The UV-unwrapped, broad-chamfer cube shown in a new project."""
    path = Path(__file__).resolve().parent.parent / "assets" / "chamfered_cube.obj"
    mesh, info = load_mesh(path, source_z_up=True)
    info.path = "Chamfered Cube"
    info.backend = "built-in"
    info.notes = []
    return mesh, info
